...
private:
	...
	void ServerSetBasePose(FGameplayTag InBasePose);

	UFUNCTION(Server, Reliable)
    void ServerSetOverlayPose(FGameplayTag InOverlayPose);

	UFUNCTION(Server, Reliable)
	void ServerSetRotation(ERotationMethod InRotationMethod, float InRotationSpeed, float InTurnStartAngle, float InTurnStopTolerance);

	UFUNCTION(Server, Unreliable)
	void ServerSetupAimOffset(EAimOffsets InAimOffsetType, EAimOffsetClamp InAimBehavior);

	UFUNCTION(Server, Unreliable)
	void ServerSetAimOffset(FRotator InAimOffset);

	UFUNCTION(NetMulticast, Unreliable)
	void NetMutiSetAimOffset(FRotator InAimOffset);

	UFUNCTION(Server, Unreliable)
	void ServerSetLookAt(FVector LookAt);

	UFUNCTION(NetMulticast, Unreliable)
	void NetMutiSetLookAt(FVector LookAt);
};
